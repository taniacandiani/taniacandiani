  ...
  static async update(id: string, project: Partial<Project>): Promise<Project | null> {
    ...
    const result = await nile.db.query(
      `UPDATE projects SET
        ...
        hero_image_descriptions = COALESCE($13::jsonb, hero_image_descriptions),
        hero_image_descriptions_en = COALESCE($14::jsonb, hero_image_descriptions_en),
        show_in_home_hero = COALESCE($15, show_in_home_hero),
        hero_description = COALESCE($16, hero_description),
        project_details = COALESCE($17, project_details),
        technical_sheet = COALESCE($18, technical_sheet),
        download_link = COALESCE($19, download_link),
        additional_image = COALESCE($20, additional_image),
        commissioned_by = COALESCE($21, commissioned_by),
        curator = COALESCE($22, curator),
        location = COALESCE($23, location),
        title_en = COALESCE($24, title_en),
        subtitle_en = COALESCE($25, subtitle_en),
        description_en = COALESCE($26, description_en),
        project_details_en = COALESCE($27, project_details_en),
        technical_sheet_en = COALESCE($28, technical_sheet_en),
        hero_description_en = COALESCE($29, hero_description_en),
        commissioned_by_en = COALESCE($30, commissioned_by_en),
        curator_en = COALESCE($31, curator_en),
        location_en = COALESCE($32, location_en),
        updated_at = NOW()
      WHERE id = $1
      RETURNING *`,
      [
