      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="space-y-6">
          ...
          <div>
            ...
            <textarea
              value={content.content.replace(/<p>/g, '').replace(/<\/p>/g, '\n\n').replace(/^\s+|\s+$/g, '')}
              onChange={(e) => {
                const paragraphs = e.target.value.split('\n\n').filter(p => p.trim());
                const htmlContent = paragraphs.map(p => `    <p>${p.trim()}</p>`).join('\n    \n');
                setContent({ ...content, content: htmlContent });
              }}
              rows={12}
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black"
              placeholder="Escribe el contenido aquí. Separa los párrafos con doble salto de línea."
            />
            <p className="text-xs text-gray-500 mt-2">
              El contenido se mostrará como párrafos en la página web
            </p>
          </div>
